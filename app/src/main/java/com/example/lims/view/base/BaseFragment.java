...
public abstract class BaseFragment<T extends ViewBinding> extends Fragment {
    ...
    private void setMIUIStatusBarDarkMode() {
        if (isMiUi) {
            Class<? extends Window> clazz = requireActivity().getWindow().getClass();
            try {
                int darkModeFlag;
                ...
                extraFlagField.invoke(requireActivity().getWindow(), darkModeFlag, darkModeFlag);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private int getStatusBarLightMode() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isMiUi) {
                result = 1;
            } else if (setMeiZuDarkMode(requireActivity().getWindow(), true)) {
                result = 2;
    ...
    protected void setStatusBarDarkMode() {
        ...
        if (type == 1) {
            ...
            setMeiZuDarkMode(requireActivity().getWindow(), true);
        } else if (type == 3) {
            requireActivity().getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
        }
    }
